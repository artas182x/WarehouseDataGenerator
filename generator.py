...
class Generator:
    ...
    def simulation(self):
        ...
        print(
            "Will be additional "
            + (days * 24 * self.config.MAX_BIKES).__str__()
            + " records"
        )
        ...
        for i in datarange(self.config.START_DATE, self.config.END_DATE, diff=1):
            ...
            self.simulate_hour(
                is_weekday,
                start_date=i,
                number_of_rented_bikes=math.ceil(
                    self.config.RENTAL_ENTRIES_PER_DAY / 24
                ),
                number_of_bikes_broken=math.ceil(
                    self.config.REPAIR_ENTRIES_PER_DAY / 48
                ),
    ...
    def reevaluate_station_states(self):
        previous_hour_states = copy.deepcopy(self._get_previous_hour_states())
        for x in previous_hour_states:
            x.date = x.date + timedelta(hours=1)
            x.id = x.id + self.config.MAX_STATIONS
    ...
    def _get_starting_stations_states(self, filter_unused, hour):
        def check_bikes_available(x):
            if 6 < hour < 20:
                return x.free_bikes > 0
            return x.free_bikes > 1
        if filter_unused:
            ...
            return [
                ...
                and check_bikes_available(x)
                and self._station_exist_in_list(x.station_id, used_stations)
            ]
        else:
            return [
                x
                for x in self.station_states[-1 * self.config.MAX_STATIONS:]
                if x.date == self._get_latest_hour() and check_bikes_available(x)
            ]
    ...
    def _get_last_station_state_by_station_id(self, station_id):
        return [
            x
            for x in self.station_states[-1 * self.config.MAX_STATIONS:]
            if x.station_id == station_id and x.date == self._get_latest_hour()
        ][0]
    ...
    def _generate_bike_repair_finish_state(self, probability_of_total_damage):
        ...
        return (
            State.WONT_REPAIR
            if 0.1 * number < probability_of_total_damage
            else State.REPAIRED
        )

    def _get_station_by_id(self, station_id):
        return [x for x in self.stations if x.station_id == station_id][0]
    ...
    def simulate_hour(
        ...
        for i in range(number_of_rented_bikes):
            ...
            starting_stations_states = self._get_starting_stations_states(
                filter_unused=self.config.STATIONS_THAT_BECAME_NOT_USED, hour=start_date.hour
            )
            if is_workday:
                start_station_state = random.choice(starting_stations_states)
            else:
                starting_stations_ids = list(map(lambda x: x.station_id, starting_stations_states))
                starting_stations = list(filter(lambda x: x.id in starting_stations_ids, self.stations))
                stations_to_rand_from = list(filter(lambda x: x.workplace is False, starting_stations))
                start_station = random.choice(stations_to_rand_from)
                start_station_state = [
                    x
                    for x in starting_stations_states
                    if x.station_id == start_station.id
                ][0]
            client = random.choice(self.clients)
            stations_to_rand_from = self._get_stations(
                filter_unused=self.config.STATIONS_THAT_BECAME_NOT_USED
            )
            if i % 3 == 0:
                stations_to_rand_from = list(filter(lambda x: x.overloaded is True, stations_to_rand_from))
            elif 8 < start_date.hour < 10 and random.randint(0, 3) != 0 and is_workday:
                stations_to_rand_from = list(filter(lambda x: x.workplace is True, stations_to_rand_from))
            elif 16 < start_date.hour < 18 and random.randint(0, 3) != 0 and is_workday:
                stations_to_rand_from = list(filter(lambda x: x.workplace is False, stations_to_rand_from))
            end_station = random.choice(
                stations_to_rand_from
            )
            bike = random.choice(
        ...
        for _ in range(number_of_bikes_serviced):
            ...
            most_frequent_station = self._get_used_by_employees_station_states(
                reverse=True
            )[0]
            bike = random.choice(
            ...
            bike.current_location = None
            least_frequent_station = self._get_used_by_employees_station_states(
                reverse=False
            )[0]
        ...
        for _ in range(number_of_bikes_broken):
            ...
            station_state = self._get_last_station_state_by_station_id(
            ...
            repair_history_entry = RepairHistory(
                self.last_repair_history_index,
                bike.id,
                workshop_name,
                State.REPAIR,
            ...
            repair_finish_state = self._generate_bike_repair_finish_state(
                probability_of_total_damage=0.05
            )

            repair_history_entry_finish = RepairHistory(
                self.last_repair_history_index,
                bike.id,
                workshop_name,
                repair_finish_state,
                repair_history_entry.date
                + timedelta(minutes=random.randint(40, self.config.BIKE_REPAIR_TIME)),
                repair_history_entry.repair_id,
            )
        ...
        for repair in local_repair_history:
            bikes_repaired = [
                x
                for x in self.bikes
                if x.id == repair.bike_id and repair.state == State.REPAIRED
            ]
    ...
    def mark_some_stations_as_overloaded(self):
        filtered_stations = list(filter(
            lambda x: x.rarely_visited is False and x.became_not_used is False,
            self.stations,
        ))
        for _ in range(4):
            station_id = filtered_stations[
                random.randint(0, len(filtered_stations) - 1)
            ].id
            for index, station in enumerate(self.stations):
                if station_id == station.id:
                    self.stations[index].overloaded = True
                    break
    ...
    def generate_stations(self, offset=0):
        ...
        self.mark_some_stations_as_overloaded()
